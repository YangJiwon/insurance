...
class UpdateContractStatusServiceTest {
	...
	class Validation {
		@Test
		@DisplayName("현재 계약 상태가 만료일때")
		void expireStatus() {
			final ContractResponse expireResponse = new ContractResponse(LocalDate.parse("2023-01-23"), contractNo, 1, ContractStatusEnum.EXPIRE.getStatus());

			given(selectService.getContractInfo(contractNo)).willReturn(expireResponse);
		...
		void validationPass() {
			final ContractResponse contractResponse = new ContractResponse(LocalDate.parse("2023-01-23"), contractNo, 1, ContractStatusEnum.NORMAL.getStatus());
